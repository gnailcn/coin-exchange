...
public class DisruptorAutoConfiguration {
    ...
    public WaitStrategy waitStrategy() {
        return new YieldingWaitStrategy();  // 自旋等待
    }
    ...
    public RingBuffer<OrderEvent> ringBuffer(
            EventFactory<OrderEvent> eventFactory,  // 事件工厂, 用于创建事件
            ThreadFactory threadFactory,    // 线程工厂, 用于创建执行者线程
            WaitStrategy waitStrategy,    // 等待策略, 用于等待ringBuffer 有数据
            EventHandler<OrderEvent>[] eventHandlers    // 事件处理器, 用于处理事件
    ) {
        ...
        // 设置消费者---我们的每个消费者代表我们的一个交易对,
        // 有多少个交易对,我们就有多少个eventHandlers ,
        // 事件来了后,多个eventHandlers 是并发执行的
