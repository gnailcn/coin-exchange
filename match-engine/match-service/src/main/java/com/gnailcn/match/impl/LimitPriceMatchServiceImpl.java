package com.gnailcn.match.impl;

import com.gnailcn.domain.ExchangeTrade;
import com.gnailcn.enums.OrderDirection;
import com.gnailcn.match.MatchService;
import com.gnailcn.match.MatchServiceFactory;
import com.gnailcn.match.MatchStrategy;
import com.gnailcn.model.*;
import com.gnailcn.rocket.Source;
...
public class LimitPriceMatchServiceImpl implements MatchService, InitializingBean {
     ...
     */
    @Override
    public void match(OrderBooks orderBooks, Order order) { //为了避免多线程环境下直接操作属性，所以传入orderBooks
        log.info("开始撮合");
        ...
        if (OrderDirection.getOrderDirection(order.getOrderDirection())  == OrderDirection.BUY) {
        ...
        while (markerQueueIterator.hasNext() && !exitLoop) {
            ...
            if (OrderDirection.getOrderDirection(order.getOrderDirection()) == OrderDirection.BUY && order.getPrice().compareTo(markerPrice) < 0) {
                break;
            }

            // 我出售一个东西 10 ,结果有个人花5块钱
            if (OrderDirection.getOrderDirection(order.getOrderDirection()) == OrderDirection.SELL && order.getPrice().compareTo(markerPrice) > 0) {
                break;
            }
        ...
        if (completedOrders.size() > 0) {
            ...
            // 7 发送盘口数据，更新盘口
            TradePlate tradePlate = OrderDirection.getOrderDirection(order.getOrderDirection()) == OrderDirection.BUY ?
                    orderBooks.getSellTradePlate() : orderBooks.getBuyTradePlate();
    ...
    private ExchangeTrade processMath(Order taker, Order marker, OrderBooks orderBooks) {
        ...
        exchangeTrade.setDirection(OrderDirection.getOrderDirection(taker.getOrderDirection()));  // 设置交易的方法
         ...
         */
        if (OrderDirection.getOrderDirection(marker.getOrderDirection()) == OrderDirection.BUY) {
            // 减少挂单的数据量
